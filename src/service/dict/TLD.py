...
class TLD(MdxService):
    ...
    def iweb_rank(self):
        ...
        if m:
            soup = parse_html(m[0])
            el_list = soup.find_all('div', class_ = 'iweb')
            if el_list:
                rank_temp = el_list[0].find_all('span',class_ = 'rank')
                if rank_temp:
                    rank=[]
                    for i in rank_temp:
    ...
    def chinese_def(self):
        ...
        if m:
            ...
            el_list = soup.find_all('div', class_ = 'coca2')
            def_distribution = ''
            if el_list:
                def_distribution = str(el_list[0])
            el_list = soup.find_all('div', class_ = 'gdc')
            cn_def = ''
            if el_list:
