...
class LLM_Chinese(WebService):
    ...
    def _query_ollama_word(self, word) -> dict:
        ...
        elif config.ollama_lang == "English":
            prompt = (
        ...
        elif config.ollama_lang == "儿童中文":
            prompt = (
                f"请用小学三年级能理解的中文解释词语 '{word}'。\n"
                f"然后请给出 3 到 5 个使用了该词语的例句。\n"
                f"注意：在详细解释和例句之间，请务必用 '{field_separator}' 作为单独的一行进行分隔。"
            )
        elif config.ollama_lang == "Child English":
            prompt = (
                f"Please explain the word '{word}' in detail using English that a 3rd-grade primary school student can understand.\n"
                f"Then, please provide 3 to 5 example sentences using this word.\n"
                f"Note: Be sure to place '{field_separator}' on its own line as a separator between the detailed explanation and the example sentences."
            )
        else:
