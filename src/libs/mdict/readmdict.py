...
class MDict(object):
    ...
    def _split_key_block(self, key_block):
        ...
        while key_start_index < block_len:
            ...
            while i < block_len:
                ...
                # If loop finishes without finding a delimiter (e.g., EOF or truncated block)
                key_end_index = block_len
                # raise ValueError('No key_end_index found.')
    ...
    def _read_records(self, build_index=False, build_index_check_block=False):
        if self._version >= 3:
            yield from self._read_records_v3(build_index, build_index_check_block)
        else:
            yield from self._read_records_v1v2(build_index, build_index_check_block)

    def _read_records_v3(self, build_index=False, build_index_check_block=False):
    ...
    def _read_records_v1v2(self, build_index=False, build_index_check_block=False):
        f = open(self._fname, 'rb')
        f.seek(self._record_block_offset)
        ...
        for compressed_size, decompressed_size in record_block_info_list:
            ...
            _current_block_type = record_block_compressed[0]
            if not build_index or build_index_check_block:
                record_block = self._decode_block(record_block_compressed, decompressed_size)
                len_record_block = len(record_block)
            else:
                record_block = b''
                len_record_block = decompressed_size

            # split record block according to the offset info from key block
            while i < len(self._key_list):
                record_start, key_text = self._key_list[i]
                # reach the end of current record block
                if record_start - offset >= len_record_block:
                    break
                # record end index
                if i < len(self._key_list)-1:
                    record_end = self._key_list[i+1][0]
                else:
                    record_end = len_record_block + offset
                ...
                if build_index:
                    ...
                    index_dict['record_block_type'] = _current_block_type
                    ...
                    self._index_list.append(index_dict)
    ...
    def get_index(self, check_block):
        self._index_list = []
        for k,v in self._read_records(True, check_block):
            pass
        print(f"MDict._index_list size {len(self._index_list)}")
